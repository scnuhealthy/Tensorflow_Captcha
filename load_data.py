...
CHAR_SET = captcha_params_and_cfg.get_char_set()

Y_LEN = captcha_params_and_cfg.get_y_len()
...
def load_data_predict(tol_num):
    ...
    # data dir
    imgs = os.listdir(captcha_params_and_cfg.data_path)
    
    for i in range(tol_num):
        # load the images and convert them into gray images
        img = get_image_from_file(captcha_params_and_cfg.data_path+'/'+imgs[i])
