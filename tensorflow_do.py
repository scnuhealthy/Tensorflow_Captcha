...
def deepnn(x):
  ...
  with tf.name_scope('fc1'):
    W_fc1 = weight_variable([15 * 15 * 64, 512])
...
# Import data
(X_train, Y_train), (X_test, Y_test) = load_data(tol_num = 10000,train_num = 8000)
print (X_train.shape)
X_train = X_train.reshape(X_train.shape[0],60,60,1)
X_test = X_test.reshape(X_test.shape[0],60,60,1)[:100]
X_train /=255
X_test /=255
Y_test = Y_test[:100]
...
with tf.Session() as sess:
	...
	for i in range(2):
		for batch_xs,batch_ys in generatebatch(X_train,Y_train,Y_train.shape[0],batch_size): 
			train_step.run(feed_dict={x: batch_xs, y_: batch_ys, keep_prob: 0.95})
			if j % 20 ==0:
